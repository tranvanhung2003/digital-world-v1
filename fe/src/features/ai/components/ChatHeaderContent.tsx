...
import { ApplyIcon, CloseIcon, LightningIcon, StatusIcon } from './icons';
  ...
  return (
    <div className="bg-gradient-to-br from-primary-500 via-primary-600 to-primary-700 text-white p-5 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20">
        <div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg%20width%3D%2240%22%20height%3D%2240%22%20viewBox%3D%220%200%2040%2040%22%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%3E%3Cg%20fill%3D%22none%22%20fill-rule%3D%22evenodd%22%3E%3Cg%20fill%3D%22%23ffffff%22%20fill-opacity%3D%220.1%22%3E%3Ccircle%20cx%3D%2220%22%20cy%3D%2220%22%20r%3D%221.5%22/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')]" />
      ...
      <div className="relative flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-green-500 hover:bg-green-600 backdrop-blur-sm flex items-center justify-center transition-all duration-200 hover:scale-105 text-sm font-medium space-x-1"
            title="Apply changes"
          >
            <ApplyIcon size={14} />
            <span>Apply</span>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2 bg-white/15 backdrop-blur-sm rounded-full px-3 py-1.5">
            <StatusIcon size={10} status={isAIReady ? 'online' : 'loading'} />
            <span className="text-xs font-semibold">
              {isAIReady ? 'Gemini AI' : 'Demo Mode'}
            </span>
          </div>
          <div className="flex items-center space-x-2 bg-white/10 backdrop-blur-sm rounded-full px-3 py-1.5">
            <span className="text-xs text-white/90">
              {isAIReady ? 'Smart Mode' : 'Limited'}
            </span>
          </div>
        </div>

        {/* Online indicator */}
        <div className="flex items-center space-x-2 bg-green-500/20 backdrop-blur-sm rounded-full px-3 py-1.5">
          <StatusIcon size={8} status="online" />
